...
2. 先读取并维护 `项目上下文.md`、`内部术语表.md`、`项目工作记录.md`。项目记忆规则由 `project-context-maintainer` 负责。
...
6. 凡涉及询问用户、让用户确认、选择方案、拍板范围或补充缺口，必须使用 AskUserQuestion 工具；不要在正文里随口问，也不要替用户拍板。
7. 每个阶段结束后必须用 AskUserQuestion 做结构化确认；未确认事项显式标记为 `【待确认：...】`，不得写成已确认结论。
...
- 任何阶段只要需要用户确认，必须先用 AskUserQuestion；未得到确认前，只能写候选方案或 `【待确认：...】`。
